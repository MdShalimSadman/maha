  ...
  try {
    ...
    if (status === "VALID" || status === "VALIDATED") {
      ...
      try {
        ...
        console.error(
          "Database Update FAILED, proceeding with redirect:",
          dbError
        );
      }

      if (orderDetails) {
        const { email, fullName, totalPrice, address, phone, paymentMethod } =
          orderDetails;
...
export async function GET(request: NextRequest) {
  ...
  if (
    tran_id &&
    order_id &&
    (status === "Success" || status === "DONE" || !status)
  ) {
    redirectUrl = `${baseUrl}/payment-success?transactionId=${tran_id}&orderId=${order_id}`;
  } else if (status === "FAILED" || status === "CANCELLED") {
    redirectUrl = `${baseUrl}/payment-failed?transactionId=${tran_id}&status=${status}&orderId=${
      order_id || ""
    }`;
  } else {
